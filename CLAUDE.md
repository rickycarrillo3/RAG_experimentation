...
- **app.py** — Gradio 6.17.3 web UI (port 7860), now an **HTTP client of `api/`**, not an importer of the pipeline. Publishes no API of its own: `analytics_enabled=False`, `footer_links` without `"api"`, and `api_visibility="private"` on every event binding, so the page cannot be used as a callable backend. ⚠️ Those last two are the **Gradio 6** spellings of what were `show_api=False` and `api_name=False` on 5.x, and the two arguments fail differently: `launch(show_api=...)` is a loud `TypeError`, while `api_name=False` is *accepted* and publishes the handler as a public endpoint named `/False` — the inverse of its old meaning. Check the result with `Blocks.get_api_info()` (empty schema) rather than by reading the argument. Uploading a file *is* the ingest trigger (`upload_box.upload`, not `.change` — the handler clears the box on failure, and `.change` would re-fire on that programmatic clear); there is no ingest button, and a failed upload asks the user to upload again. Kept because a working UI on the real API proves the API is complete before any TypeScript exists; delete it when the TS frontend lands rather than porting it. Keeps a separate "clean" conversation history that it sends back as context, distinct from the display history: token frames flagged `server_marker` (the sources footer, the truncation notice) are shown to the student but kept out of it, so the model is never handed a transcript in which it appears to have written the server's words.
