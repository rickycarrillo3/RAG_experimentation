...
- **config.py** — **everything that changes between machines, in one place**: index locations (`DATA_DIR`, `CHROMA_DIR`, `BM25_DIR`), where Ollama is (`OLLAMA_BASE_URL`), how the app serves (`APP_HOST`, `APP_PORT`, `APP_AUTH`), and device selection (`resolve_device()`, `REQUIRE_GPU`). Every default reproduces the original local-Mac behaviour, so importing it changes nothing until an env var is set. Moving to a new box is an environment change, never a code edit — **put new deployment knobs here, not as literals in the module that happens to need them.**
  - `resolve_device()` returns `"cuda"` when available and `None` otherwise (leaving sentence-transformers' own detection in charge, which picks MPS on the Mac). It is the single guard against the pipeline's nastiest silent failure: a CPU fallback is indistinguishable from success — the app starts and answers, just 10-40x slower. `REQUIRE_GPU=1` turns that into a startup crash.
- **ingest.py** — chunks `.mmd` files via `chunking.CHUNKERS[--chunker]` (default `baseline`) and builds two indexes per user: a BM25 pickle (`$DATA_DIR/bm25_indexes/user_<name>.pkl`, `{"bm25": BM25Okapi, "chunks": [...]}`) and a Chroma collection (`$DATA_DIR/chroma_db/`, collection `user_<name>`). `--embed-model` / `--normalize-latex` select the embedder via `retrieval.load_embeddings` (no more duplicate model constant); returns build stats for the sweep. It re-exports `BM25_DIR`/`CHROMA_DIR` (which it imports from `retrieval`) only because `evaluation/embed_chunk_sweep.py` imports them *from this module* — they are **not** defined here; there is one definition, in `config.py`.
...
- **app.py** — Gradio **5.50** web UI (`APP_HOST`:`APP_PORT`, default `0.0.0.0:7860`). The 5.x pin is forced, not incidental: `marker-pdf` requires `transformers<5`, which requires `huggingface-hub<1.0`, while Gradio 6 requires `huggingface-hub>=1.2` — the ranges are disjoint, so gradio and the extraction stack cannot both be current. Gradio gave way because holding transformers/surya still keeps extraction and retrieval behaviour identical, and so keeps the recorded eval numbers comparable. Two lines are Gradio-5-specific and load-bearing: **`gr.Chatbot(type="messages")`** (5.x still defaults to the legacy tuple format and rejects the dicts `handle_chat` yields; Gradio 6 *removed* the parameter, so it raises `TypeError` there), and **`theme` on `gr.Blocks(...)` rather than `launch()`** (Gradio 6 tolerated `launch(theme=…)`, Gradio 5 raises `TypeError`; `Blocks(theme=…)` is the documented spelling in both). Raising the pin back to 6 means revisiting the first of those in the same commit. Imports the pipeline from `retrieval.py`; loads embeddings/reranker/LLM once at module level. `APP_AUTH` ("user:pass" pairs) puts a Gradio login in front of the app; unset, it prints a loud warning and serves with no login at all. **That login gates access to the app, not to documents** — per-user isolation is still just a lowercased username string typed into a textbox, so anyone who logs in can read any user's documents by typing their name. Falls back to the model's own knowledge if a user has no uploaded documents. Keeps a separate "clean" conversation history (sources stripped) for LLM context, distinct from the display history.
