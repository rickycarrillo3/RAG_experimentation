...
- **telemetry.py** — append-only JSONL event log (`$KBM_DATA_DIR/telemetry/events.jsonl`): one `query` record per answer (hashed user, mode, retrieved chunk ids/scores, per-stage timings) plus `feedback` records keyed by `event_id`. Exists to be *already collecting* by the time it's needed: logged questions are the real query distribution that `EVALUATION.md §8` names as the gap synthetic gold sets cannot close, and thumbs-up (question, chunk) pairs are the contrastive data for the embedding fine-tune (`ROADMAP §6`). Never let a telemetry failure break a query.
