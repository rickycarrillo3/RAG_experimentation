...
# Closed-book: 20 hand-written questions, no retrieval — this measures deepseek-math, not the pipeline.
python evaluation/self_consistency.py                   # greedy + k=1/5/10 → evaluation/results/self_consistency.json
...
- **evaluation/self_consistency.py** — does majority voting over k sampled answers beat one greedy answer? Runs `evaluation/reasoning_set.jsonl` (20 hand-written closed-ended questions with verifiable answers) **closed-book — no retrieval**, because the subject is the generator's reasoning stability and routing it through retrieval would make a wrong answer un-attributable. Draws N samples per question **once** and estimates accuracy at k=1/5/10 by resampling that pool, so every k is scored on identical outputs and k=1 isn't a single-sample coin flip; greedy (temperature 0) is scored separately as the baseline that actually ships. Answers are normalized to numbers before voting (`5/16` and `0.3125` must be one vote, not two). The output that decides the question is the ERROR STRUCTURE block, not the accuracy table: errors where ≥80% of samples agree on the same wrong answer are **stable** and no amount of sampling fixes them. See `EVALUATION.md §11`.
